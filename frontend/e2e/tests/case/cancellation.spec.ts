...
import { loginAs, logout, selectDropdownOption, completeStage, fillStageChecklist, openStage, gotoCaseDetail, cancelCase } from '../../helpers/case-workflow-helpers';
...
test.describe('Case Cancellation', () => {
  ...
  test('CS can cancel case at Stage 3 when cost is rejected', async ({ page, request }) => {
    ...
    // CS can cancel case after cost rejection
    await logout(page);
    await loginAs(page, TEST_USERS.CS, TEST_USERS.PASSWORD);
    await gotoCaseDetail(page, testCaseId);
  ...
  test('Cannot edit cancelled case', async ({ page, request }) => {
    await loginAs(page, TEST_USERS.CS, TEST_USERS.PASSWORD);
    const testCaseId = await createCase(page);
  ...
  test('Cannot redo cancelled case', async ({ page, request }) => {
    await loginAs(page, TEST_USERS.CS, TEST_USERS.PASSWORD);
    const testCaseId = await createCase(page);
    ...
    const redoButton = page.locator('button:has-text("Redo")').or(page.locator('button:has-text("redo")'));
  ...
  test('Only CS can cancel case when cost is rejected', async ({ page, request }) => {
    await loginAs(page, TEST_USERS.CS, TEST_USERS.PASSWORD);
    const testCaseId = await createCase(page);
